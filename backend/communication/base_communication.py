...
class BaseCommunication:
    ...
    def _write_config(self) -> bool:
        ...
        try:
            ...
            workstation_use_request = [0,0,0,0,0,0]
            workstation_use_ids = []
            ...
            for workstation_config_data in workstation_use_configs:
                ...
                workstation_id= workstation_config.get("workstation_id",0)
                if workstation_config == {} or workstation_id==0:
                    log.warning(f"workstation_config is invalid workstation_id = {workstation_id}")
                    continue
                workstation_use_request[workstation_id-1]=1
            ...
            ws_next_interval=[part_start_to_ws1_interval,0,0,0,0,0,0]
            camera_reset_time=[0,0,0,0,0,0]
            ws_seq_count=[0,0,0,0,0,0]
            ...
            for workstation_config_data in workstation_use_configs:
                ...
                if workstation_config == {} or workstation_id==0:
                    log.warning(f"workstation_config is invalid workstation_id = {workstation_id}")
                    continue
                index=workstation_id-1
                ws_next_interval[workstation_id]=int(workstation_config.get("to_next_ws_offset", 0))
                camera_reset_time[index]=int(workstation_config.get("camera_reset_time", 0))
                ...
                ws_seq_count[index]=workstation_config.get("sequence_count", 0)
                ...
                current_offset_address=start_offset_address+index*120
                current_repeat_address=start_repeat_address+index*120

                log.debug(f"_write_workstation_repeat:address 1_{current_repeat_address} ,data {repeat_list}")
                for value in repeat_list:
                    self.snap_client.write(address=f"1_{current_repeat_address}", value=value, datatype='int32')
                    current_repeat_address+=4

                log.debug(f"_write_workstation_offset:address 1_{current_offset_address} ,data {sequences_intervals}")
                for value in sequences_intervals:
                    self.snap_client.write(address=f"1_{current_offset_address}", value=value, datatype='int32')
                    current_offset_address+=4
